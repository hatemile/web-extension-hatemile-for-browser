function initHaTeMiLeForBrowser() {
    ...
    var accessibleEvent = (
        new hatemile.implementation.AccessibleEventImplementation(htmlParser)
    );
    ...
    var accessibleForm = (
        new hatemile.implementation.AccessibleFormImplementation(htmlParser)
    );
    ...
    if (typeof hatemile_for_browser_configuration !== typeof undefined) {
        var configure = new hatemile.util.Configure(
            hatemile_for_browser_configuration
        );
        ...
        var validateFields = htmlParser.find(
            '[data-changeadded]'
        ).listResults();
        for (i = 0, length = validateFields.length; i < length; i++) {
            ...
            field.addEventListener('change', function(event) {
                var target = (
                    new hatemile.util.html.vanilla.VanillaHTMLDOMElement(
                        event.target
                    )
                );
                ...
                for (var j = 0, len = customAttributes.length; j < len; j++) {
                    ...
                    if (!target.hasAttribute(customAttribute)) {
                        var invalidDisplay = htmlParser.find(
                            '['
                            + customAttribute
                            + 'beforeof="'
                            + id
